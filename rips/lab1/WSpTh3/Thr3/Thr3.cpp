...
void TerminateX(HWND hWnd, HMENU hMenu, BOOL * fTerminateE);
...
//As it is named it is possible to verify if there is only one application instance too!
	...
	if (!InitializeCriticalSectionAndSpinCount(&CriticalSection,
		0x00000400)) {
		MessageBox(NULL, "Unable to inicialize critical section", "Four Threads",
			MB_OK | MB_ICONEXCLAMATION);
		return 01;
	}
...
LPVOID wait(DWORD& dwRet, int i, TCHAR szRetRes[100], std::string error) {
	...
	case WAIT_ABANDONED_0:
		...
		MessageBox(NULL, szRetRes, "WaitForSingleObject", MB_OK | MB_ICONEXCLAMATION);
		break;
	case WAIT_FAILED:
...
LONG WndProc_OnDestroy(HWND hWnd) {
	...
	for (int i = 0; i < 3; i++) {
		wait(dwret, i, szRetRes, error);
	}
	
	
	if (!fTerminateE && !CloseHandle(hThreadE[0])) {
		MessageBox(NULL, "Unable to close", "Elipse", MB_OK | MB_ICONEXCLAMATION);
	}
	if (!fTerminateR && !CloseHandle(hThreadE[1])) {
		MessageBox(NULL, "Unable to close", "Rectangle", MB_OK | MB_ICONEXCLAMATION);
	}
	if (!fTerminateX && !CloseHandle(hThreadE[2])) {
		MessageBox(NULL, "Unable to close", "X", MB_OK | MB_ICONEXCLAMATION);
	}
...
unsigned int __stdcall  PaintX(void* hWnd) {
	...
	while (!fTerminateX) {
		...
		xRight = (rect.right + 1);
		yTop = rand() % (rect.top + 1);
		yBottom = (rect.bottom + 1);
		double r = rand()% (min(xRight-xLeft, yBottom-yTop) + 1);
